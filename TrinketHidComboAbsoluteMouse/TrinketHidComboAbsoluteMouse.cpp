...
void Trinket_Hid_Combo::mouseMove(int16_t x, int16_t y, uint8_t buttonMask)
{
	report_buffer[1] = buttonMask;
	report_buffer[0] = REPID_MOUSE;
	report_buffer[2] = x & 0xff;
	report_buffer[3] = (x >> 8) & 0xff;
	report_buffer[4] = y & 0xff;
	report_buffer[5] = (y >> 8) & 0xff;
